    ...
    <>
      ...
      >
        ...
        <div className="relative w-full px-6 lg:px-12 flex items-center justify-between gap-4">
          ...
          >
            <span className="font-geist-mono uppercase text-[0.62rem] sm:text-[0.7rem] tracking-[0.3em] text-white/80 group-hover:text-white transition-colors">
              {abierto ? 'Cerrar' : 'Menú'}
            </span>

            {/* Dos trazos que se cruzan al abrir. El de abajo no desaparece
                encogiéndose a cero: se va desplazado, que es como se mueven las
                cosas de verdad. */}
            <span aria-hidden className="relative block w-8 h-3">
              <span
                className={cn(
                  'absolute left-0 w-full h-px bg-white transition-transform duration-300 ease-out-quart',
            ...
            {paginasConfig.map((p, i) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="relative font-geist-mono text-[0.58rem] tracking-[0.28em] text-exvia-red-text transition-colors duration-300 group-hover:text-exvia-red">
                    {String(i + 1).padStart(2, '0')}
                  </span>

                  {/* El texto se vuelve negro sobre el hueso: la fila queda en
                      negativo. Negro sobre #FBF7F5 son 19:1 de contraste.
                  ...
                  <span
                    className={cn(
                      'relative font-display uppercase leading-none tracking-[-0.01em] text-[clamp(1.5rem,3.2vw,2.6rem)] transition-colors duration-300 group-hover:text-black',
                      activa ? 'text-exvia-red-text' : 'text-[#FBF7F5]'
                    )}
                  >
                    {p.etiquetaCorta}
                  </span>
